...
class MyAdvancedSearchTool:
    ...
    def _setup_search_sources(self):
        ...
        if os.getenv("SERPAPI_API_KEY"):
            try:
                ...
                print("[OK] SerpApi搜索源已启用")
            except ImportError:
                print("⚠️ SerpApi库未安装")

        if self.search_sources:
            print(f"[TOOL] 可用搜索源: {', '.join(self.search_sources)}")
        else:
    ...
    def search(self, query: str) -> str:
        """执行智能搜索"""
        if not query.strip():
            return "[ERROR] 错误：搜索查询不能为空"

        # 检查是否有可用的搜索源
        if not self.search_sources:
            return """[ERROR] 没有可用的搜索源，请配置以下API密钥之一：
...
配置后重新运行程序。"""
        ...
        return "[ERROR] 所有搜索源都失败了，请检查网络连接和API密钥配置"
